import { ICreateAccount, IEmailTemplate, IResetPassword } from '../types/emailTamplate';

const LOGO_URL = `10.10.7.111:5000/uploads/emobold.logo.png`;

const createAccount = (values: ICreateAccount) => {
  const data = {
    to: values.email,
    subject: 'Verify your Duty Hour Tracking Account',
    html: `<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; margin: 50px; padding: 20px; color: #555;">
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <img src="" alt="Logo" style="display: block; margin: 0 auto 20px; width:150px" />
          <h2 style="color: #3b7a87; font-size: 24px; margin-bottom: 20px;">Hey! ${values.name}, Welcome to Duty Hour Tracking App</h2>
        <div style="text-align: center;">
            <p style="color: #555; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">Your single use code is:</p>
            <div style="background-color: #3b7a87; width: 80px; padding: 10px; text-align: center; border-radius: 8px; color: #fff; font-size: 25px; letter-spacing: 2px; margin: 20px auto;">${values.otp}</div>
            <p style="color: #555; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">This code is valid for 3 minutes.</p>
        </div>
    </div>
</body>`,
  };
  return data;
};

const resetPassword = (values: IResetPassword) => {
  const data = {
    to: values.email,
    subject: 'Reset your Duty Hour Tracking Account password',
    html: `<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; margin: 50px; padding: 20px; color: #555;">
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <img src="https://i.postimg.cc/6pgNvKhD/logo.png" alt="Logo" style="display: block; margin: 0 auto 20px; width:150px" />
        <div style="text-align: center;">
            <p style="color: #555; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">Your single use code is:</p>
            <div style="background-color: #3b7a87; width: 80px; padding: 10px; text-align: center; border-radius: 8px; color: #fff; font-size: 25px; letter-spacing: 2px; margin: 20px auto;">${values.otp}</div>
            <p style="color: #555; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">This code is valid for 3 minutes.</p>
            <p style="color: #b9b4b4; font-size: 16px; line-height: 1.5; margin-bottom: 20px;text-align:left">If you didn't request this code, you can safely ignore this email. Someone else might have typed your email address by mistake.</p>
        </div>
    </div>
</body>`,
  };
  return data;
};



// employee email


const employeeEmailTemplate = (values: IEmailTemplate) => {
  const data = {
    to: values.email,
    subject: 'Welcome to Duty Hour Tracking App',
    html: `
    <body style="font-family: Arial, sans-serif; background-color: #f9f9f9; margin: 50px; padding: 20px; color: #555;">
      <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        
        <img src="${LOGO_URL}" alt="Logo" style="display: block; margin: 0 auto 20px; width:150px" />
        
        <h2 style="color: #3b7a87; font-size: 24px; margin-bottom: 20px; text-align:center;">
          Hey ${values.name}, Welcome to Duty Hour Tracking App
        </h2>

        <div style="text-align: center;">
          
          <p style="color: #555; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">
            Your employee account has been created successfully.
          </p>

          <p style="color: #555; font-size: 16px; line-height: 1.5;">
            Your temporary password is:
          </p>

          <div style="background-color: #3b7a87; width: 180px; padding: 12px; text-align: center; border-radius: 8px; color: #fff; font-size: 20px; letter-spacing: 2px; margin: 20px auto;">
            ${values.password}
          </div>

          <p style="color: #555; font-size: 14px; line-height: 1.5;">
            Please login and change your password after your first login.
          </p>

        </div>

      </div>
    </body>
    `,
  };

  return data;
};


export const emailTemplate = {
  createAccount,
  resetPassword,
  employeeEmailTemplate
};
